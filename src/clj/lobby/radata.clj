...
(def data (atom {
  ...
  :magicitems [
    ...
    {:id 6 :name "Research"       :type "magicitem" :action [{:exhaust true :cost {:any 1} :draw 1}]}
  ...
  :monuments [
    ...
    {:id 3 :name "Hanging Gardens"  :type "monument" :cost {:gold 4} :collect [{:any 3}] :vp 1}
    {:id 4 :name "Library"          :type "monument" :cost {:gold 4} :action [] :vp 1}
    {:id 5 :name "Mausoleum"        :type "monument" :cost {:gold 4} :action [] :vp 2}
    {:id 6 :name "Obelisk"          :type "monument" :cost {:gold 4} :bought {:collect {:any 6}} :vp 1}
    {:id 7 :name "Oracle"           :type "monument" :cost {:gold 4} :action [] :vp 1}
    {:id 8 :name "Solomon's Mine"   :type "monument" :cost {:gold 4} :action [] :vp 1}
    {:id 9 :name "Temple"           :type "monument" :cost {:gold 4} :collect [{:life 1}] :action [] :vp 1}
  ]
  :placesofpower [
    {:id 6 :base 6 :name "Sacred Grove"          :type "pop" :fg? true :cost {:life 8 :calm 4} :action [] :vp nil}
    {:id 0 :base 6 :name "Alchemists Tower"      :type "pop" :cost {:gold 3} :collect [{:any 3}] :action [] :vp nil}
                                                        
    {:id 1 :base 1 :name "Catacombs of the Dead" :type "pop" :fg? true :cost {:death 9} :collect [{:death 1}]:action [] :vp nil}
    {:id 7 :base 1 :name "Sacrificial Pit"       :type "pop" :cost {:elan 8 :death 4} :action [] :vp nil}
                                                        
    {:id 3 :base 3 :name "Cursed Forge"          :type "pop" :fg? true :cost {:elan 5 :life 5 :calm 5} :collect [{:discard {:death 1}} {:exhaust 1}] :action [] :vp nil}
    {:id 5 :base 3 :name "Dwarven Mines"         :type "pop" :cost {:elan 4 :life 2 :gold 1} :collect [{:gold 1}] :action [] :vp nil}
                                                        
    {:id 2 :base 2 :name "Coral Castle"          :type "pop" :fg? true :cost {:elan 3 :life 3 :calm 3 :death 3} :action [] :vp nil}
    {:id 9 :base 2 :name "Sunken Reef"           :type "pop" :cost {:calm 5 :elan 2 :life 2} :collect [{:gold 1}] :action [] :vp nil}
                                                        
    {:id 4 :base 4 :name "Dragon's Lair"         :type "pop" :fg? true :cost {:elan 6 :death 3} :action [] :vp nil}
